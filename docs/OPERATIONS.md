...
- [x] Health check externo em `https://astryxion.com/health`.

### Governança de Dados (LGPD)

- **Encarregado de Dados (DPO):** Equipe de Privacidade (`support@astryxion.com`).
- **Direito ao Esquecimento:** Implementado via funcionalidade "Self-Service" no Developer Portal (`Danger Zone`). 
    - O processo é automatizado e revoga todas as chaves de API instantaneamente antes de deletar o registro do usuário.
    - Caso o usuário não tenha acesso ao portal, o admin pode realizar o processo manualmente via terminal/Redis.
- **Retenção:** Logs de API são rotacionados/anonimizados após 90 dias. Dados de cálculos esotéricos são eliminados do cache em 24h.
