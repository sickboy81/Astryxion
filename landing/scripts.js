...
async function loadLandingPricing() {
    ...
    try {
        ...
        tiers.forEach((tier) => {
            ...
            const tag = tier.tagline != null && String(tier.tagline).trim();
            if (tag) {
                const tagEl = document.createElement('p');
                tagEl.className = 'tier-tagline';
                tagEl.textContent = String(tier.tagline).trim();
...
// Cookie Consent Banner Injection & Logic (LGPD)
const initCookieBanner = () => {
    if (localStorage.getItem('astryxion_cookies_accepted')) return;
    ...
    banner.innerHTML = `
        <div class="container cookie-content">
            <div class="cookie-text">
                ...
                <p>We use functional cookies to enhance your experience. By continuing, you agree to our <a href="/privacy.html">Privacy Policy</a>.</p>
            </div>
            <div class="cookie-actions">
                <button id="accept-cookies" class="btn btn-secondary btn-sm">Accept All</button>
            </div>
        </div>
    `;
    ...
    document.getElementById('accept-cookies').addEventListener('click', () => {
        localStorage.setItem('astryxion_cookies_accepted', 'true');
