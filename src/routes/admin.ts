...
import { fetchMergedTiersConfig, listAllTiersForAdmin, listPublicTiersOrdered } from "../lib/tier-plans.js";
import os from "node:os";
import { getBrowserPoolStatus, resetBrowserPool } from "../lib/pdf-engine.js";
import { UserRecord } from "../lib/user-store.js";
...
const AdminUpdateTierSchema = z.object({
  ...
  features: z.array(z.string().max(800)).max(40).optional(),
  tagline: z.string().max(4000).optional(),
...
export async function registerAdminRoutes(app: FastifyInstance, apiKeyStore: any, userStore: any, redis: any, serviceName: string, port: number, startedAtUtc: string, instanceId: string) {
  ...
  app.get("/admin/api/config/tiers", { schema: { hide: true } }, async (req, reply) => {
    ...
    const tiers = await listAllTiersForAdmin(redis);
    return { tiers };
  });
  ...
  app.post("/admin/api/config/tiers", { schema: { hide: true } }, async (req, reply) => {
    ...
    const merged = await fetchMergedTiersConfig(redis);
    const existing = merged[tierId];
    if (!existing) return reply.code(400).send(errorBody("VALIDATION_ERROR", "Tier inválido."));
    ...
    const updated: TierConfig = {
      ...
      features: parsed.data.features !== undefined ? parsed.data.features : existing.features,
      tagline: parsed.data.tagline !== undefined ? parsed.data.tagline : existing.tagline,
      enabled: parsed.data.enabled !== undefined ? parsed.data.enabled : existing.enabled !== false,
    };
  ...
  // --- USER & USAGE ADMINISTRATION ---

  app.get("/admin/api/users", { schema: { hide: true } }, async (req, reply) => {
    const authErr = await ensureAdmin(req, reply);
    if (authErr) return authErr;
  ...
  app.get("/admin/api/usage/global", { schema: { hide: true } }, async (req, reply) => {
    const authErr = await ensureAdmin(req, reply);
    if (authErr) return authErr;

    const usageByHash: Record<string, { today: number, week: number, month: number }> = {};
    let totalToday = 0;
    let totalWeek = 0;
    let totalMonth = 0;
    ...
    if (redis) {
      ...
      const todayBucket = formatDate(today);
      ...
      for (let i = 0; i < 30; i++) {
        ...
        const b = formatDate(d);
        if (i < 7) weekBuckets.add(b);
        monthBuckets.add(b);
      }

      let cursor = "0";
      const pattern = `rate:*:*`; // Scan all rate keys
      ...
      do {
        ...
        if (keys.length > 0) {
          ...
          for (let i = 0; i < keys.length; i++) {
            ...
            const part = keys[i].split(":");
            const hash = part[1];
            const b = part[2];

            if (!usageByHash[hash]) usageByHash[hash] = { today: 0, week: 0, month: 0 };

            if (b === todayBucket) {
              usageByHash[hash].today += val;
              totalToday += val;
            }
            if (weekBuckets.has(b)) {
              usageByHash[hash].week += val;
              totalWeek += val;
            }
            if (monthBuckets.has(b)) {
              usageByHash[hash].month += val;
              totalMonth += val;
            }
          }
        }
    ...
    return { totalToday, totalWeek, totalMonth, usageByHash };
  });
}
