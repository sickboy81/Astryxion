...
export async function registerPortalMgmtRoutes(app: FastifyInstance, apiKeyStore: any, userStore: any, redis: any) {
  ...
  // 7. Permanent Account Deletion (LGPD/GDPR)
  app.delete("/api/v1/portal/account", { schema: { hide: true } }, async (req, reply) => {
    const session = await ensureUser(req, reply);
    if (!session) return;

    app.log.warn({ email: session.email }, "User initiated permanent account deletion");

    // A. Delete all API keys associated with this user
    const allKeysRaw = await apiKeyStore.getAllRecords();
    const userHashes = Object.entries(allKeysRaw)
      .map(([hash, json]) => {
        const rec = jsonParseSafe<ApiKeyRecord>(json as string);
    ...
    for (const hash of userHashes) {
      await apiKeyStore.deleteRecord(hash);
      if (redis) await redis.sadd("api_keys:blacklist", hash);
    }

    // B. Delete the user account
    await userStore.deleteUser(session.email);

    // C. Invalidate current session
    const authHeader = req.headers.authorization!;
    const token = authHeader.substring(7);
    await userStore.logout(token);

    app.log.info({ email: session.email }, "Account deletion complete");
    return { ok: true, message: "Account and all associated keys have been permanently deleted." };
  });
}
