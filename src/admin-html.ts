...
<body>
    ...
    <div class="sidebar">
        ...
        <a href="#" class="nav-item" onclick="showTab('users', this)">Registered Users</a>
        ...
        <div id="tab-overview" class="admin-tab active-tab">
            <div class="grid">
                ...
                <div class="card" style="border-color: var(--info);">
                    <h3 style="color: var(--info);">Requests Today</h3>
                    <div class="value" id="stat-usage-today">--</div>
                </div>
                <div class="card" style="border-color: var(--warning);">
                    <h3 style="color: var(--warning);">Requests (7d)</h3>
                    <div class="value" id="stat-usage-week">--</div>
                </div>
                <div class="card" style="border-color: var(--success);">
                    <h3 style="color: var(--success);">Requests (30d)</h3>
                    <div class="value" id="stat-usage-month">--</div>
                </div>
            </div>
        </div>
    ...
    <div class="modal" id="planModal">
        <div class="modal-content" style="max-width: 640px; max-height: 90vh; overflow-y: auto;">
            <h2>Edit API Plan</h2>
            <p style="font-size: 12px; color: var(--text-dim); margin: -8px 0 16px;">These texts appear under each plan on the <strong>landing page</strong> and the <strong>customer portal</strong>.</p>
            ...
            <div class="form-group">
                <label>Short text under price (optional)</label>
                <textarea id="edit-plan-tagline" rows="2" placeholder="e.g. Best for indie apps" style="width: 100%; background: rgba(0,0,0,0.2); border: 1px solid var(--border); color: white; padding: 10px 12px; border-radius: 6px; resize: vertical; font-family: inherit;"></textarea>
            </div>
            <div class="form-group">
                <label>Bullet list <span style="opacity:0.7;font-weight:400">(one line = one bullet)</span></label>
                <textarea id="edit-plan-features" rows="10" placeholder="One feature per line" style="width: 100%; background: rgba(0,0,0,0.2); border: 1px solid var(--border); color: white; padding: 10px 12px; border-radius: 6px; resize: vertical; font-family: inherit; font-size: 13px;"></textarea>
            </div>
            <div class="form-group">
    ...
    <script>
        ...
        async function loadData() {
            try {
                const [stats, usageData, keysData] = await Promise.all([
                    api('/admin/api/stats'),
                    api('/admin/api/usage/global'),
                    api('/admin/api/keys')
                ]);
                ...
                document.getElementById('stat-usage-today').textContent = (usageData.totalToday || 0).toLocaleString();
                document.getElementById('stat-usage-week').textContent = (usageData.totalWeek || 0).toLocaleString();
                document.getElementById('stat-usage-month').textContent = (usageData.totalMonth || 0).toLocaleString();

                const tbody = document.getElementById('keys-table-body');
                tbody.innerHTML = '';
                
                keysData.keys.forEach(k => {
                    const u = usageData.usageByHash[k.hash] || { today: 0, week: 0, month: 0 };
                    ...
                    tr.innerHTML = \`
                        ...
                        <td><strong style="color: \${u.today > 0 ? 'var(--info)' : 'var(--text-dim)'}">\${u.today.toLocaleString()}</strong></td>
                        <td><strong style="color: \${u.month > 0 ? 'var(--info)' : 'var(--text-dim)'}">\${u.month.toLocaleString()}</strong></td>
        ...
        async function loadUsers() {
            try {
                ...
                data.users.forEach(u => {
                    const tr = document.createElement('tr');
                    tr.innerHTML = \`
                        <td style="font-size: 11px; opacity: 0.6; font-family: monospace;">\${u.id}</td>
                        <td style="font-weight: 500;">\${u.email}</td>
                        <td><span class="badge \${u.role === 'admin' ? 'admin' : 'basic'}">\${u.role}</span></td>
                        <td style="font-size: 12px; color: var(--text-dim);">\${new Date(u.createdAt).toLocaleString()}</td>
                        <td><span style="text-transform: uppercase; font-size: 11px;">\${u.preferredLang || 'en'}</span></td>
                    \`;
                    tbody.appendChild(tr);
                });
        ...
        function openPlanModal(id) {
            ...
            document.getElementById('edit-plan-tagline').value = (p.tagline != null && p.tagline !== undefined) ? String(p.tagline) : '';
            const feats = Array.isArray(p.features) ? p.features : [];
            document.getElementById('edit-plan-features').value = feats.join('\\n');
        ...
        async function savePlan() {
            ...
            const tagline = document.getElementById('edit-plan-tagline').value;
            ...
            try {
                await api('/admin/api/config/tiers', {
                    method: 'POST',
                    body: JSON.stringify({ id, displayName, requestsPerDay, price, tagline, features, enabled })
                });
