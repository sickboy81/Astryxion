...
export function createUserStore(redis: Redis | null) {
  ...
  async function deleteUser(email: string): Promise<void> {
    const userKey = `auth:users:${email}`;
    if (redis) {
      await redis.del(userKey);
    }
    inMemoryUsers.delete(userKey);
  }
  ...
  async function listAllUsers(): Promise<UserRecord[]> {
    ...
    if (redis) {
      ...
      do {
        ...
        if (keys.length > 0) {
          ...
          for (const val of values) {
            if (val) {
              const u = jsonParseSafe<UserRecord>(val);
              if (u) users.push(u);
            }
          }
        }
      ...
      for (const val of inMemoryUsers.values()) {
        const u = jsonParseSafe<UserRecord>(val);
        if (u) users.push(u);
      }
    }
  ...
  return { getUser, registerUser, login, getSession, logout, setupDefaultAdmin, updateUser, deleteUser, listAllUsers };
}
