...
<head>
    ...
    <style>
        ...
        * { margin:0; padding:0; box-sizing:border-box; }
        ...
        .tier-tagline { font-size: 13px; color: var(--text-dim); line-height: 1.45; margin: 0 0 14px; }
    ...
    <div class="container">
        ...
        <div class="grid">
            ...
            <div class="side-content">
                ...
                <div class="section-title" style="color: var(--error);">⚠️ Danger Zone</div>
                <div class="card" style="border-color: rgba(255, 62, 62, 0.3);">
                    <p style="font-size: 11px; color: var(--text-dim); margin-bottom: 15px;">Permanently delete your identity and all associated network data.</p>
                    <button class="btn btn-danger" style="width: 100%;" onclick="deleteAccount()">Delete Account</button>
                </div>
            </div>
        </div>
    </div>
    ...
    <script>
        ...
        function renderTiers(tiers) {
            ...
            container.innerHTML = tiers.map(function(t) {
                var tag = (t.tagline && String(t.tagline).trim())
                    ? '<p class="tier-tagline">' + escTierHtml(String(t.tagline).trim()) + '</p>'
                    : '';
                var feats = Array.isArray(t.features) ? t.features : [];
                var listInner;
                if (feats.length > 0) {
                    listInner = feats.map(function(f) { return '<li>' + escTierHtml(f) + '</li>'; }).join('');
                } else {
                    var n = typeof t.requestsPerDay === 'number' ? t.requestsPerDay : Number(t.requestsPerDay);
                    var q = Number.isFinite(n) ? n.toLocaleString() : String(t.requestsPerDay);
                    listInner = '<li>' + escTierHtml('Up to ' + q + ' requests per day') + '</li>';
                }
                return '<div class="tier-card ' + (t.id === highlightId ? "active" : "") + '">' +
                    '<div class="tier-name">' + escTierHtml(t.displayName) + '</div>' +
                    '<div class="tier-price">' + escTierHtml(t.price) + '</div>' +
                    tag +
                    '<ul class="tier-features">' + listInner + '</ul>' +
                '</div>';
            }).join('');
        }
        ...
        async function deleteAccount() {
            const confirmed = confirm("⚠️ CRITICAL ACTION: Are you sure you want to permanently delete your account?\\n\\nThis will revoke all API keys instantly and purge your data from the grid. This action is irreversible.");
            if (!confirmed) return;

            const secondConfirm = confirm("FINAL WARNING: Do you really want to proceed with account deletion?");
            if (!secondConfirm) return;

            try {
                const res = await fetch('/api/v1/portal/account', {
                    method: 'DELETE',
                    headers: { 'Authorization': 'Bearer ' + sessionToken }
                });
                const data = await res.json();
                if (res.ok) {
                    alert("Account purged successfully. Redirecting...");
                    localStorage.removeItem("astro_session_token");
                    location.href = "/";
                } else {
                    alert("Deletion failed: " + (data.message || "Grid error"));
                }
            } catch (e) {
                alert("Network failure during deletion handshake.");
            }
        }
    </script>
</body>
